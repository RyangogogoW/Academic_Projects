...
public class StarterTests {
	 ...
	 * possible before the submission deadline.)
	 * 
	 * Inputs: An array `a` of integers and an integer `target` Output: A boolean
	 * `result` Problem: Given an array `a` of integers, compute recursively and
	 * return a boolean `result` which indicates whether or not it's possible to
	 * choose a group of some of the integers in `a`, such that this chosen group of
	 * integers sums up to the given `target`.
	 * 
	 ...
	 * possible before the submission deadline.)
	 * 
	 * Inputs: An integer `h` (height of a staircase) and an integer `n` (maximum
	 * steps of each climb) Output: An integer `result` Problem: Given `h` and `n`,
	 * compute recursively and return an integer `result` which indicates the number
	 * of possible ways for climbing a staircase of height `h`, while each climb is
	 * **up to** `n` steps.
	 * 
	...
	public void testTask3_01() {
		RecursiveMethods rm = new RecursiveMethods();
		/*
		 * There is only 1 possible way for climbing a staircase of height 4, while each
		 * climb takes 1 step. 1. 1 step, 1 step, 1 step, 1 step
		 */
		assertEquals(1, rm.task3(99, 1));
	}
	...
	public void testTask3_02() {
		RecursiveMethods rm = new RecursiveMethods();
		/*
		 * There are 5 possible ways for climbing a staircase of height 4, while each
		 * climb takes either 1 or 2 steps. 1. 1 step, 1 step, 1 step, 1 step 2. 1 step,
		 * 1 step, 2 steps 3. 1 step, 2 steps, 1 step 4. 2 steps, 1 step, 1 step 5. 2
		 * steps, 2 steps
		 */
	...
	public void testTask3_03() {
		...
		/*
		 * There are 5 possible ways for climbing a staircase of height 4, while each
		 * climb takes either 1, 2, or 3 steps. 1. 1 step, 1 step, 1 step, 1 step 2. 1
		 * step, 1 step, 2 steps 3. 1 step, 2 steps, 1 step 4. 1 step, 3 steps 5. 2
		 * steps, 1 step, 1 step 6. 2 steps, 2 steps 7. 3 steps, 1 step
		 */
	 ...
	 * possible before the submission deadline.)
	 * 
	 * Inputs of this task are the same as those of Task 3, with the same
	 * assumptions: n <= h, each climb takes at least 1 step.
	 * 
	 * Output of this task is a HashSet, where each element is an ArrayList. Each
	 * ArrayList encodes a strategy for climbing the staircase.
	 */
